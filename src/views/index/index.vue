  ...
  <div class="index-page">
    ...
    <div class="detail">
      ...
      <template v-if="cur_memoid">
        <cus-editor v-model="content" @update:modelValue="contentChange(cur_memoid, content)">
          <input
            placeholder="输入标题"
            class="memo-title"
            v-model="title"
...
import Cataloges from './cataloges.vue'
...
const updateCtx = () => {
  let cur_memo = store.activeMemos.find((row) => row.memo_id == cur_memoid.value)
  if (cur_memo) {
    // debugger
    // 避免数据修改页面不更新
    nextTick(() => {
